...
class AccountLoginEmailNewPasswordIncorrect extends Test
{
    public function __construct()

    {
        parent::__construct("Incorrect login", "Use email and incorrect password for login");
    }
    public function run(array $resultValues): bool
    {
        ...
        if (!curl_errno($ch)) {
            ...
            if ($data["ok"]) {
                $this->ansi->color(SGR::COLOR_FG_RED)
                    ->text('Reponse is a positive, it shold not be')
                    ->nostyle()->lf()->bell()->lf();
                return false;
            }
