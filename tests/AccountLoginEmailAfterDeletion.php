...
class AccountLoginEmailAfterDeletion extends Test
{
    public function __construct()

    {
        parent::__construct("Login after deletion", "Use email for login after deletion");
    }
    public function run(array $resultValues): bool
    {
        ...
        if (!curl_errno($ch)) {
            ...
            if ($data["ok"]) {
                $this->ansi->color(SGR::COLOR_FG_RED)
                    ->text('Reponse should not be positive')
                    ->nostyle()->lf()->bell()->lf();
                return false;
            }
